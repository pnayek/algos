...
public class LongestAbsoluteFilePath {
	 ...
	 * 
	 * Leetcode warm up contest #388, Medium
	 */
		...
		for (String s : vals) {
			...
			while (true) {
				...
				if (v.length == size + 1) {
					...
					int fl = name.length() + l + 1; // path/name
					// if file check if it exceeds max
					if (name.indexOf('.') >= 0) {
						// file
						max = fl > max ? fl : max;
					}
						...
						list.add(size + 1); list.add(fl);
						System.out.println("Pushing " + name + ", length: " + fl);
						stack.push(list);
					}
					break;
				}
